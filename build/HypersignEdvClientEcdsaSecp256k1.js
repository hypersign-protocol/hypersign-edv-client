...
class HypersignEdvClientEcdsaSecp256k1 {
    ...
    async allowIndexing(vaultId, allow, account, invoker) {
        const msgParams = {
            domain: {
                name: 'Allow Indexing',
                version: '1',
    ...
    async getHmacSha256Key2020(account, vaultId, invoker) {
        const hexSignature = await this.allowIndexing(vaultId, true, this.verificationMethod.blockchainAccountId.split(':')[2], invoker);
        return {
    ...
    async registerEdv(config) {
        ...
        if (!this.shaHmacKey2020) {
            this.shaHmacKey2020 = await this.getHmacSha256Key2020(this.verificationMethod.blockchainAccountId.split(':')[2], edvConfig.id, edvConfig.invoker);
        }
        ...
        const resp = await utils_1.default._makeAPICall({
            url: edvRegisterURl,
            method: 'POST',
            body: edvConfig,
            headers,
    ...
    async createCanonicalRequest({ url, method, query, headers, body }) {
        let action = 'read';
    ...
    async signRequest({ url, method, query, headers, body, keyId }) {
    ...
    async sign(canonicalRequest, walletAddress, walletType) {
        let signature;
        switch (walletType) {
    ...
    async signWithMetamask(canonicalRequest, walletAddress) {
        // @ts-ignore
        if (!window.ethereum) {
    ...
    async encryptDocument({ document, recipients }) {
     ...
     */
    async insertDoc({ document, documentId, sequence, edvId, metadata, recipients, indexs, }) {
        if (recipients) {
            ...
            recipients.push({
                id: this.keyAgreement?.id,
                type: this.keyAgreement?.type,
                encryptionPublicKeyBase64: (0, exports.multibaseBase58ToBase64)(this.keyAgreement?.id.split('#')[1]),
            });
        }
        let finalIndex;
        if (indexs) {
            const hmac = await Hmac_1.default.create({
     ...
     */
    async updateDoc({ document, documentId, sequence, edvId, metadata, indexs, }) {
     ...
     */
    async fetchDoc({ documentId, edvId, sequence, }) {
    ...
    async decryptDocument({ encryptedDocument, recipient, }) {
        // @ts-ignore
        // const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });
    ...
    // }
    async decrypt(encryptedMessage, keyId) {
    ...
    async fetchAllDocs({ edvId, limit, page }) {
        if (!limit)
            limit = 10;
        if (!page)
            page = 1;
