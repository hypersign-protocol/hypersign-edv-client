...
class HypersignCipher {
    ...
    async _getX25519KeyAgreementKey(keyAgreementKey = this.keyAgreementKey) {
        if (keyAgreementKey.type === Types_1.VerificationKeyTypes.Ed25519VerificationKey2020) {
            const ed25519KeyPair = await ed25519_verification_key_2020_1.Ed25519VerificationKey2020.generate({ ...keyAgreementKey });
    ...
    async _getX25519KeyAgreementResolver(keyResolver = this.keyResolver, id) {
    ...
    async filterRecipients(recipients) {
    ...
    async encryptObject({ plainObject, recipients = [], keyResolver, keyAgreementKey = this.keyAgreementKey, }) {
    ...
    async decryptObject({ jwe, keyAgreementKey = this.keyAgreementKey }) {
        const x25519keyAgreementKey = await this._getX25519KeyAgreementKey(keyAgreementKey);
